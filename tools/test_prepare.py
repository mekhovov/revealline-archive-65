...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (1120, 594895002))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.96.0'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-65/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], '526e900abd6ca6c375d4de67f3cef424ac040279')
        self.assertEqual(release['sourceRevision'], '781e1c3b82236003bf8600a2cac0b5022e4298c1')
        self.assertEqual(release['sourceTree'], '37030950049bdc752eff86cb9664b12e6f74c0c0')
        self.assertEqual(release['sourceQualification'], {
            'bytes': 14164,
            'sha256': '4f7c8d5ff983128bd3562c6633160d0702e1f3b732edbd8939df7e7077afb7b4',
        ...
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.96.0/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (1117, 594893608))
